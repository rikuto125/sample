...
export const STAGES: Stage[] = [
  ...
  // ---- ch4-s1: 注文確定から出荷まで（正常系・越境の順番）----
  {
    ...
    modeLabel: 'MODE 1 タイムライン',
    scenario:
      '顧客が注文を確定した。在庫集約はその事実を受けて在庫を引き当て、引き当てが成功すると出荷準備が整い、商品が出荷された。注文集約の出来事と在庫集約の出来事が、2つの集約をまたいで連なる正常フロー。',
    instruction: '2つの集約をまたぐ事実を時系列に。越境の「順番」を掴もう',
    events: [
    ...
    reality: {
      ...
      long: '2つの集約を1トランザクションで更新するか分けるかは設計判断（DDD-FAQ §5.2.1）。分けるなら、注文の事実に在庫が反応する＝イベント＋ポリシーで繋ぐ。次のステージで「失敗したら？」を見る。',
    },
  },
  ...
  // ---- ch4-s2: 在庫が足りなかったら（補償フロー）----
  {
    ...
    reality: {
      short:
        '集約をまたいだロールバックは無い。確定した事実は消えず、「取り消された」という打ち消す事実（補償イベント）を積む。',
      long: 'これが結果整合性。在庫集約の「引き当てに失敗した」という失敗イベントに、注文集約側の補償ポリシーが反応して「注文を取り消す」。2相コミットで強整合を取らず、失敗を事実として扱い補償で整える——分散した集約・サービスをまたぐときの定石（Saga）。',
    },
  },
  ...
  // ---- ch4-s3: 補償が連鎖する（決済も巻き戻す）----
  {
    ...
    distractors: [
      ...
      },
      {
        ...
        isDistractor: true,
        reason:
          '「決済代行サービス」は置けません — それは外部システム（ピンク）。外側の住人で、時間軸の事実ではありません。',
      },
    ],
    orderConstraints: [
    ...
    reality: {
      ...
      long: 'Saga は「どこまで進んだか」を覚えていて、失敗地点から逆順に補償を流す。EventStorming でこの長い補償フローを付箋で可視化すると、現場の「失敗時にどこまで巻き戻すか」という Pain Point が露わになる。',
    },
  },
    ...
    distractors: [
      {
        ...
        isDistractor: true,
        reason:
          '現在形は意図＝コマンド（水色）です。起きた事実なら「支払われた」のはず。',
      },
      {
        id: 's5a-d-serve',
        kind: 'command',
        ...
        isDistractor: true,
        reason:
          '現在形は意図＝コマンド（水色）です。起きた事実なら「提供された」のはず。',
      },
    ],
    orderConstraints: [
  ...
  //   DDD-FAQ §8.4 軽量クエリモデルの範囲（厳密 CQRS/イベントソーシングは扱わない）。
  // ============================================================
  ...
  // ---- ch6-s1: この画面はどの事実でできている？----
  {
    ...
    reality: {
      ...
      long: 'DDD-FAQ §8.4 はこれを軽量CQRS（クエリモデル）と呼ぶ。表示用に“集約”を作るのは定義違反——参照用モデルとして分け、複数集約/複数イベントから join して導く。EventStorming では緑の付箋が「誰がこの画面を、どの事実から見たいのか」を炙り出す。',
    },
  },
  ...
  // ---- ch6-s2: 1つの事実が複数の画面を更新する ----
  {
    ...
    commands: [
      ...
      { id: 's6b-rm-sales', kind: 'readModel', labelJa: '売上サマリ画面' },
    ...
    reality: {
      short:
        '事実は1つでも、それを見たい画面は複数。だから読モは「どの画面が、どの事実を必要とするか」で設計する。',
      long: '§8.4「参照系を全てクエリモデルにする必要はない」——必要なところだけ。EventStorming では緑の付箋を最後にまとめて貼り、「この事実を見て誰が何を判断するか」を問うと、抜けていた画面要件が出てくる。',
    },
  },

  // ---- ch6-s3: 読モは集約ではない（更新と参照を分ける）----
  {
    id: 'ch6-s3',
    mode: 'trigger',
    name: '読モは集約ではない',
    ...
    modeLabel: 'MODE 2 リードモデル',
    scenario:
      '総仕上げ。コマンドは集約を更新し（青→集約）、リードモデルは事実から導かれる（緑←イベント）。更新用モデル（集約）と参照用モデル（リードモデル）は別物だ。事実とリードモデルの導出関係を最後に締めくくる。',
    instruction:
    ...
    triggers: [
      ...
      { id: 's6c-e-delivered', kind: 'event', labelJa: '配達が完了した' },
      {
        ...
        labelJa: '注文',
        reason:
          '集約（淡黄）は“更新用モデル”。リードモデルは集約そのものではなく、集約が出した事実から導かれる別物です（§8.4）。',
      },
      {
        id: 's6c-d-order',
        kind: 'command',
    ...
    reality: {
      short:
        '更新（コマンド→集約）と参照（イベント→リードモデル）は別の道。同じ“注文”でも、更新する集約と見るための読モは別物。',
      long: 'DDD-FAQ §8.4「集約は必ずまとめて更新するまとまり。表示用の集約は定義違反」。読モは複数集約をまたいで join して導く参照専用モデル。次は実ワークショップで、自分の業務の画面要件を緑の付箋で“どの事実から導かれるか”として並べてみよう。',
    },
  },
]
